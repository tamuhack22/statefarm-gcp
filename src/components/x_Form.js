...
export default class Form extends React.Component {
  ...
  submitCallback() {
    ...
    var bmi_age = bmi*age
    formDataSerialized["bmi"] = bmi
    formDataSerialized["bmi_age"] = bmi_age

    // this.cf.addRobotChatResponse("Thank you {name}!, we are analyzing your info!")
    
    // this.setState({isLoading: true}, ()=>{
      // this.sendFormData(formDataSerialized)
    // })
  ...
  sendFormData = (formData) => {
    ...
    return
    axios.post(`http://104.154.20.69:5000/predict`, { formData })
      .then(res => {
    ...
    // setTimeout(()=>{
    //   this.setState({isLoading: false, isSubmitted: true, apiResponse: formData})
    // }, 3000)
  }
  
  render() {
    const { isSubmitted, isLoading, apiResponse } = this.state;
